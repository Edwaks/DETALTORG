...
**DETALTORG** is a business marketplace for ordering electronic components. It is designed for procurement departments, engineers, and component suppliers and organizes purchases of rare and production-volume parts through structured requests and relevant offers.

- Public URL: https://detaltorg.ru/
- Core entities: components, specifications, requests, requirement lines, offers, and orders.
- Key capabilities: component catalog, part-number search, supply request, batch of line items, supplier offers, order approval, request history.

## Why the Product Is Needed

A real workflow rarely consists of a single action. Users must understand the initial situation, gather information, choose the next step, coordinate a decision, and return to the result later. DETALTORG provides one domain-specific environment for this work. It preserves context between stages and shows not only an individual item, but also its place in the overall process.

The product’s main value is not the number of screens, but workflow continuity. Components, specifications, requests, requirement lines, offers, and orders are treated as parts of one user journey. New participants understand the current state faster, experienced specialists spend less time matching information manually, and managers gain a clear overview without requiring every step to be retold.
...
The roles Buyer, Engineer, and Supplier work with the same domain model from different perspectives. Consistent names for objects and states reduce ambiguity during handoffs. A responsible participant can reference a specific object and expected result instead of repeating the entire context.

## Main Sections

The public description covers these user capabilities:

- **Component catalog** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Part-number search** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Supply request** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Batch of line items** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Supplier offers** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Order approval** — a distinct part of the workflow, connected to the product model and a clear user outcome.
- **Request history** — a distinct part of the workflow, connected to the product model and a clear user outcome.
...
### Detaltorg home
![Detaltorg home](docs/screenshots/detaltorg_home.png)

### Detaltorg home full
![Detaltorg home full](docs/screenshots/detaltorg_home_full.png)

### Detaltorg catalog
![Detaltorg catalog](docs/screenshots/detaltorg_catalog.png)
